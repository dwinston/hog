...
def roll_dice(num_rolls, dice=six_sided):
    ...
    rolls = [dice() for roll in range(num_rolls)]
...
def take_turn(num_rolls, opponent_score, dice=six_sided):
    ...
    if num_rolls == 0:
        return 1 + largest_digit(opponent_score)
    else:
        return roll_dice(num_rolls, dice)

def largest_digit(integer):
    """Get the largest digit in the integer.
...
def play(strategy0, strategy1, goal=GOAL_SCORE):
    ...
    while score < 100 and opponent_score < 100:
        ...
        num_rolls = strategy[who](score, opponent_score)
        score += take_turn(num_rolls, opponent_score, dice)
        # "Current" player switches after each iteration of this loop, and thus
        # scores swap, unless there's a swine swap to cancel that.
        if not (score == 2 * opponent_score or 2 * score == opponent_score):
...
def max_scoring_num_rolls(dice=six_sided):
    ...
    high = (0, 0) # num_dice, turn_score
    for num_dice in range(1,11): # 1,2,..,10
        turn_score = averaged(num_dice, dice)
        print('{0} dice scores {1} on average'.format(num_dice, turn_score))
        high = (num_dice, turn_score) if turn_score > high[1] else high
    return high[0]
...
def bacon_strategy(score, opponent_score):
    ...
    0
    """
    if 1 + largest_digit(opponent_score) >= BACON_MARGIN:
        return 0
    else:
...
def swap_strategy(score, opponent_score):
    ...
    5
    """
    free_bacon = 1 + largest_digit(opponent_score)
    score_after_roll_0 = score + free_bacon
    if beneficial_swap(score_after_roll_0, opponent_score):
        return 0
...
def harmful_swap(score, opponent_score):
    return score == 2 * opponent_score

def final_strategy(score, opponent_score):
    """When to take bacon, and the number of rolls if not taking bacon.

    If taking the bacon will result in a beneficial swap, do it. If it will
    result in a harmful swap, don't take it. Otherwise, if the quantity of
    free bacon is large enough, or if taking the bacon will force four-sided
    dice on the opponent, take the bacon.

    If not taking the bacon, roll a number of dice that will maximize
    the expected score for the type of dice used (four-sided or six-sided).

    The above results in a win rate of 0.5890 (1e6 averaged runs), so need to
    do better to get a win rate >= 0.59 as required.

    Additional strategy:
    If in the lead, accept a lower bacon margin because the reward of a high
    roll is no longer worth the risk of "pig out".

    Now, the win rate is 0.5977 (1e6 averaged runs). Great!
    """
    # Max-scoring num_rolls for four-sided dice is 4, and max-scoring num_rolls
    # for six-sided dice is 6 (result of experiment with 1e6 averaged runs).
    num_rolls = 4 if hog_wild(score, opponent_score) else 6

    # Can decrease bacon margin if in lead
    bacon_margin = 8
    if score > opponent_score + (0.2 * GOAL_SCORE):
        bacon_margin = 6
    elif score > opponent_score + (0.1 * GOAL_SCORE):
        bacon_margin = 7
    ...
    if beneficial_swap(score_after_roll_0, opponent_score):
        ...
        return 0
    elif hog_wild(score_after_roll_0, opponent_score):
        # Take the bacon and thus force four-sided dice on opponent
        return 0
    else:
        return num_rolls
